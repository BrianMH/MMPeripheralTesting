...
void MPU6500_WriteToReg(uint8_t address, uint8_t data) {
	GPIO_ResetBits(_CS_GROUP, _CS_PIN);

	HUSART_writeString(USART3, "Waiting for tx buffer...\n");
	while(!SPI_I2S_GetFlagStatus(_SPI_CHANNEL, SPI_I2S_FLAG_TXE));	// wait until tx buffer empty
	HUSART_writeString(USART3, "Sending packet...");
	SPI_I2S_SendData(_SPI_CHANNEL, address);	// send a packet
   HUSART_writeString(USART3, "Sent packet\n");
	while(!SPI_I2S_GetFlagStatus(_SPI_CHANNEL, SPI_I2S_FLAG_RXNE)); // wait until rx buffer not empty
	SPI_I2S_ReceiveData(_SPI_CHANNEL);
	HUSART_writeString(USART3, "Received packet\n");

	while(!SPI_I2S_GetFlagStatus(_SPI_CHANNEL, SPI_I2S_FLAG_TXE));
	SPI_I2S_SendData(_SPI_CHANNEL, data);
   HUSART_writeString(USART3, "Sent packet\n");
	while(!SPI_I2S_GetFlagStatus(_SPI_CHANNEL, SPI_I2S_FLAG_RXNE));
	SPI_I2S_ReceiveData(_SPI_CHANNEL);
   HUSART_writeString(USART3, "Received packet\n");

	GPIO_SetBits(_CS_GROUP, _CS_PIN);
}
...
uint8_t MPU6500_ReadFromReg(uint8_t address) {
	...
	while(!SPI_I2S_GetFlagStatus(_SPI_CHANNEL, SPI_I2S_FLAG_TXE));
	SPI_I2S_SendData(_SPI_CHANNEL, 0x80|address);
   HUSART_writeString(USART3, "Sent packet\n");
	while(!SPI_I2S_GetFlagStatus(_SPI_CHANNEL, SPI_I2S_FLAG_RXNE));
	SPI_I2S_ReceiveData(_SPI_CHANNEL); //Clear RXNE bit
   HUSART_writeString(USART3, "Received packet\n");

	while(!SPI_I2S_GetFlagStatus(_SPI_CHANNEL, SPI_I2S_FLAG_TXE));
	SPI_I2S_SendData(_SPI_CHANNEL, 0x00); //Dummy byte to generate clock
   HUSART_writeString(USART3, "Sent dummy packet\n");
	while(!SPI_I2S_GetFlagStatus(_SPI_CHANNEL, SPI_I2S_FLAG_RXNE));
   HUSART_writeString(USART3, "Received packet\n");

	GPIO_SetBits(_CS_GROUP, _CS_PIN);
	return SPI_I2S_ReceiveData(_SPI_CHANNEL);
}
...
void MPU6500_Init() {
	...
	// Init clocks
	RCC_AHB1PeriphClockCmd(_CLOCKS_INIT, ENABLE);
	RCC_APB2PeriphClockCmd(_SPI_RCC_CLOCK, ENABLE);
	...
	GPIO_Init(_CLOCK_GROUP, &GPIO_InitTypeDefStruct);

   GPIO_InitTypeDefStruct.GPIO_Pin = _DATAIN_PIN;
   GPIO_Init(_DATAIN_GROUP, &GPIO_InitTypeDefStruct);

   GPIO_InitTypeDefStruct.GPIO_Pin = _DATAOUT_PIN;
   GPIO_Init(_DATAOUT_GROUP, &GPIO_InitTypeDefStruct);

	GPIO_PinAFConfig(_CLOCK_GROUP, _CLOCK_SOURCE, _SPI_ALT_DERIV);
	GPIO_PinAFConfig(_DATAIN_GROUP, _DATAIN_SOURCE, _SPI_ALT_DERIV);
	GPIO_PinAFConfig(_DATAOUT_GROUP, _DATAOUT_SOURCE, _SPI_ALT_DERIV);

	// and set up SS pin
	GPIO_InitTypeDefStruct.GPIO_Pin = _CS_PIN;
	GPIO_InitTypeDefStruct.GPIO_PuPd = GPIO_PuPd_UP;
	GPIO_InitTypeDefStruct.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_Init(_CS_GROUP, &GPIO_InitTypeDefStruct);
	GPIO_SetBits(_CS_GROUP, _CS_PIN);

	GPIO_InitTypeDefStruct.GPIO_Pin = _FSYNC_PIN;
	GPIO_Init(_FSYNC_GROUP, &GPIO_InitTypeDefStruct);
	GPIO_ResetBits(_FSYNC_GROUP, _FSYNC_PIN);
	...
	SPI_Init(_SPI_CHANNEL, &SPI_InitTypeDefStruct);
	SPI_Cmd(_SPI_CHANNEL, ENABLE);

	// Places a delay for the loop to set itself up and then allow reading from device
	TIM_Delay_Micro(1000);
   HUSART_writeString(USART3, "Waiting for device...\n");
