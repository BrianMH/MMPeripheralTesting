...
/**
 * A method of writing unsigned integers to the serial part. Just in case the
 * unsigned int may be larger than the max integer.
 * @param usart - the serial peripheral to send data from
 * @param twrite - a pointer to the buffer containing the stream
 * @return true if the transfer was successful, false otherwise
 */
bool HUSART_writeUInt(USART_TypeDef* usart, uint32_t twrite);
