...
//   USART_DeInit(usart);
   ...
   if(usart == USART2) {      // USART2 ON PA2/PA3
   ...
   } else if(usart == USART3) {     // USART3 ON PB10/PB11
      // Initialize corresponding clocks
      ...
      GPIO_InitStructureTx.GPIO_Pin = GPIO_Pin_11;
      ...
      // Initializes the structs meant for this application
      RingBuffer_init(&rx3Buf);
      RingBuffer_init(&tx3Buf);
   }
...
void USART3_IRQHandler(void) {
   if(USART_GetITStatus(USART3, USART_IT_RXNE) == SET)
      // Place new char in rx buffer
      RingBuffer_queue(&rx3Buf, USART_ReceiveData(USART3));
   if(USART_GetITStatus(USART3, USART_IT_TXE) == SET) {
      // Send more characters from buffer
      if(RingBuffer_isEmpty(&tx3Buf))
         USART_ITConfig(USART3, USART_IT_TXE, DISABLE);  // empty. done sending
      else
         USART_SendData(USART3, RingBuffer_dequeue(&tx3Buf)); // still have more to send
   }
}
